...
if (-Not $Force) {
    do {
        $YesOrNo = Read-Host "Are you sure you want to remove all Office versions from this PC? (y/n)"
...
if (-not ($RunAgain)) {
    if (Test-Path "HKEY_LOCAL_MACHINE\SOFTWARE\OEM\M365\Install") {
        $CurrentStageValue = (Get-ItemProperty "HKEY_LOCAL_MACHINE\SOFTWARE\OEM\M365\Install").CurrentStage
